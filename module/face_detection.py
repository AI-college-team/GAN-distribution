...
def face_points_detection(img, bbox:dlib.rectangle):
    ...
    coord1 = np.asarray(list([p.x, p.y] for p in shape.parts()[27:36]), dtype=np.int)
    coord2 = np.asarray(list([p.x, p.y] for p in shape.parts()[0:17]), dtype=np.int)
    coord3 = np.asarray(list([p.x, p.y] for p in shape.parts()[48:61]), dtype=np.int)

    coords = np.concatenate((coord1,coord2))
    coords = np.concatenate((coords,coord3))
