...
class GAN_Service:
    def __init__(self):
        print("\n------Initiating crop modul------\n")
        self.face_detector = get_detector("resnet50_2020-07-20", max_size=800)
        self.mask_classifier = get_classifier("tf_efficientnet_b0_ns_2020-07-29")
        self.face_detector.eval()
        self.mask_classifier.eval()

        print("\n------Initiating Detector------\n")
        self.state_D = torch.load('/home/gan_unmask/GAN_Service/module/detect_b32_123e.pth', map_location='cpu')
        self.trained_Dmodel = detect_model.Detector() 
        self.trained_Dmodel.load_state_dict(self.state_D)

        print("------Initiating Editor------\n")
        self.state_G = torch.load('/home/gan_unmask/GAN_Service/module/Emodel_our_116e.pth', map_location='cpu')
        self.trained_Gmodel = edit_model.GatedGenerator()
        self.trained_Gmodel.load_state_dict(self.state_G['G'])
        
        print("\n######_We're_all_set_######\n\n")

    # 이미지 input 부분
    def goGan(self, maskedimg):
        img_gt = maskedimg
        ...
        #croplist
        with torch.no_grad():
            annotations = self.face_detector.predict_jsons(img_gt)
        ...
        with torch.no_grad():
            for annotation in tqdm(annotations):
                ...
                predictions += [self.mask_classifier(model_input.unsqueeze(0))[0].item()]
        ...
        for i in range(length):
            ...
            temp_b = self.trained_Dmodel.forward(temp_m)
            ...
            # Editor 모델에 넣고 후처리
            first_out, second_out = self.trained_Gmodel(temp_m, after_sharpen)
    ...
    #### face_swap용 함수 
    def goGanSwap(self, maskedimg, unmaskedimgpath):
        img_gt = maskedimg
        img_gt = cv2.cvtColor(img_gt, cv2.COLOR_BGR2RGB)
        h_gt,w_gt,_1=img_gt.shape
        ...
        #####swap###########
        orderpath = '/home/gan_unmask/GAN_Service/module/swap_main.py'
        unmaskedpath = unmaskedimgpath
        ganpath = '/home/gan_unmask/GAN_Service/img_temp/gtemp0.jpg'
        outputpath = '/home/gan_unmask/GAN_Service/img_temp/gtemp0.jpg'
     
        command = f'python3 {orderpath} --src {unmaskedpath} --dst {ganpath} --out {outputpath} --correct_color'
        os.system(command)
        ...
        img_gt2.save('/home/gan_unmask/GAN_Service/static/after/after_my_'+f'{wholelen+1}'.zfill(5)+'.jpg')
